...
```bash
bun scripts/detect_slop.ts <file> [--verbose]
```
...
```bash
# Preview changes
bun scripts/clean_slop.ts <file>

# Apply changes (creates backup)
bun scripts/clean_slop.ts <file> --save

# Aggressive mode (may slightly change meaning)
bun scripts/clean_slop.ts <file> --save --aggressive
```
...
bun scripts/clean_slop.ts <file> --output clean_file.txt
...
bun scripts/detect_slop.ts src/documentation.md --verbose

# Clean up automatically
bun scripts/clean_slop.ts src/documentation.md --save
```
...
2. Run: bun scripts/detect_slop.ts article.txt --verbose
3. Review findings and apply manual cleanup
4. Optionally run: bun scripts/clean_slop.ts article.txt --save
